...
const developer = employee.filter((employees) => {
  return employees.position === "Developer";
});
console.log(developer);

const female = employee.filter((employees) => {
  return employees.gender === "Female";
});
console.log(female);

const highSalary = employee.filter((employees) => {
  return employees.salary > 2000;
});
console.log(highSalary);

const employeeNophoneNomail = employee.filter((employees) => {
  const { email, phoneNumber } = employees;
  return !email && !phoneNumber;
});
console.log(employeeNophoneNomail);

const employeeHighSalaryNoMail = employee.filter((employees) => {
  const { email, salary } = employees;
  return !email && salary > 2000;
});
console.log(employeeHighSalaryNoMail);

const trackingEmployee = employee.map((employees) => {
  return (employees = employees.name);
});
console.log(trackingEmployee);

const idAndNameList = employee.map((employees) => {
  const id = employees.id;
  const name = employees.name;
  return { listInfo: `id: ${id}, name: ${name}` };
});
console.log(idAndNameList);

const upperPosition = employee.map((employees) => {
  return {
    ...employees,
    position: employees.position.toUpperCase(),
  };
});
console.log(upperPosition);

const generatedEmail = employee.map((employees) => {
  const id = employees.id;
  const position = employees.position;
  if (employees.email === null)
    return { ...employees, email: `${id}_${position}@gmail.com` };
  return employees;
});
